    ...
    >
      <div className="relative overflow-hidden aspect-[3/4] bg-[#f5f5f5]">
        <img
          ...
          src={getImageUrl(hovered && hasSecondImage ? image[1] : image[0])}
          alt={name}
        />
        <button
          onClick={handleWishlist}
          className={`absolute top-3 right-3 z-10 w-8 h-8 rounded-full flex items-center justify-center transition bg-white/80 backdrop-blur-sm ${
            wishlisted ? "text-black" : "text-gray-400 hover:text-black"
          }`}
        >
