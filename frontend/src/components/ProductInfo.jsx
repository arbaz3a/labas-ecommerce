  ...
  return (
    <div className="flex-1 flex flex-col gap-6">
      ...
      <p className="text-xl font-medium">
        {currency}
        {productdata.price.toLocaleString("en-PK", { minimumFractionDigits: 2 })}
      </p>
...
<div>
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs font-medium uppercase tracking-widest text-gray-500">Size</p>
          {productdata.category !== "Watches" && productdata.category !== "Fragrances" && (
            <button
              onClick={() => setShowSizeChart(true)}
              className="flex items-center gap-1 text-xs text-gray-500 hover:text-black transition cursor-pointer"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 7h6m-6 4h3m-3 4h6M5 3h14a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2z" />
              </svg>
              Size Guide
            </button>
          )}
        ...
        <div className="flex gap-2 flex-wrap">
          {productdata.sizes.map((size) => (
            <button
              ...
              className={`px-4 py-2.5 text-xs transition cursor-pointer outline-none border ${selectedSize === size
...
<div className="flex gap-3 mt-2">
        <button
          onClick={() => {
            toast.dismiss();
            if (!selectedSize) {
              toast("Please select a size", { type: "error", toastId: "size-error-toast" });
              return;
            }
            addToCart(productdata._id, selectedSize, quantity);
            toast("Added to cart", { type: "success", toastId: "cart-success" });
          }}
        ...
        <button
          ...
          className={`w-14 flex items-center justify-center border transition cursor-pointer ${wishlisted
...
<div className="border-t pt-2">
        ...
        <div className={`grid transition-all duration-300 ease-in-out ${openDesc ? "grid-rows-[1fr] opacity-100 mt-3" : "grid-rows-[0fr] opacity-0"}`}>
...
<div className="border-t pt-2">
        ...
        <div className={`grid transition-all duration-300 ease-in-out ${openCare ? "grid-rows-[1fr] opacity-100 mt-3" : "grid-rows-[0fr] opacity-0"}`}>
          <div className="overflow-hidden">
