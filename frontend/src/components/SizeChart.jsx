...
<div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                                <h2 className="text-lg font-semibold text-black" style={{ fontFamily: "'Georgia', serif" }}>
                                    {chart.title}
                                </h2>
                                <button onClick={onClose} className="p-1 text-gray-400 hover:text-black transition cursor-pointer">
...
<div className="px-6 py-5">
                                <table className="w-full text-left">
                                    <thead>
                                        <tr>
                                            {chart.headers.map((h) => (
                                                <th key={h} className="text-[11px] font-semibold tracking-wider uppercase text-gray-500 pb-3 pr-4">
                                                    {h}
                                                </th>
                                            ))}
                                        </tr>
                                    ...
                                    <tbody>
                                        {chart.rows.map((row, i) => (
                                            <tr key={i} className="border-t border-gray-100">
                                                {row.map((cell, j) => (
                                                    <td key={j} className={`py-3 pr-4 text-sm ${j === 0 ? "font-semibold text-black" : "text-gray-600"}`}>
                                                        {cell}
                                                    </td>
                                                ))}
                                            </tr>
...
{chart.note && (
                                <div className="px-6 pb-6">
                                    <h3 className="text-xs font-bold tracking-wider uppercase mb-2 text-black">How to Measure</h3>
                                    <p className="text-sm text-gray-500 leading-relaxed">{chart.note}</p>
                                </div>
                            )}
                        </div>
                    </motion.div>
